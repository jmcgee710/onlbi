...
const routesToPrerender = [
  '/', '/beaches', '/towns', '/lbi-map', '/lbi-town-boundaries', '/lbi-town-services',
  '/lbi-conditions', '/accessibility', '/eat', '/do', '/getting-around',
  ...towns.map(t => `/${t}`),
  ...sections.map(s => `/${s}`),
...
const routeMeta = route => {
  ...
  if (route === '/lbi-map') return { changefreq: 'weekly', priority: '0.9' }
  if (route === '/lbi-town-boundaries') return { changefreq: 'monthly', priority: '0.8' }
  if (route === '/lbi-town-services') return { changefreq: 'monthly', priority: '0.8' }
